...
\cite{plett_ekf} & Plett & Model-Based (ECM $+$ Extended Kalman Filter) \\
\cite{he_calce} & He et al. & Model-Based (Empirical/Double-Exponential; source of the CALCE CS2 data) \\
\cite{ma_dual_exp} & Yuan et al. & Model-Based (Empirical/Double-Exponential $+$ EKF) \\
\cite{verhulst_pso} & Xian et al. & Model-Based (Empirical/Verhulst $+$ Particle Filter) \\

% Data-Driven Examples
\cite{severson_data} & Severson et al. & Data-Driven (large-fleet cycle-life prediction) \\
\cite{naseri_efficient} & Qin et al. & Data-Driven (gradient boosting, BMS-oriented) \\
\cite{zhang_rnn} & Naghashi et al. & Data-Driven (RNN, time-series forecasting) \\
...
\cite{raissi_pinn} & Raissi et al. & Hybrid (foundational PINN framework) \\
\cite{wang_pinn} & Xue et al. & Hybrid (PDE-based PINN, SPM) \\
\cite{pde_pinn} & Wen et al. & Hybrid (ODE-regularized PINN, uncertainty-weighted) \\
\cite{ref1_general} & Madani et al. & Comprehensive Review (All Categories) \\
\cite{berecibar_review} & Berecibar et al. & Review (SOH estimation methods) \\
\cite{saxena_metrics} & Saxena et al. & Evaluation methodology (prognostic metrics) \\
