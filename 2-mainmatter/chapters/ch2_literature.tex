...
\textbf{Limitations:} Despite their accuracy, Transformers have a quadratic memory complexity ($O(L^2)$) with respect to sequence length, making them difficult to deploy on resource-constrained hardware compared to the linear complexity ($O(L)$) of RNNs \cite{machine_learning_web}. Equally important, their headline accuracy is earned on \emph{large} training corpora: self-attention models are notoriously data-hungry, and in low-data regimes their excess capacity turns into overfitting rather than accuracy. This matters for the BMS setting, where run-to-failure data for a given cell design is scarce --- a point this thesis tests empirically by benchmarking two Transformer scales under realistic data volumes (Chapter~\ref{ch:results}). Furthermore, purely data-driven models like SGEformer are prone to overfitting to dataset artifacts, such as sensor noise masquerading as capacity regeneration, leading to physically implausible predictions (e.g., predicting infinite battery life in certain local minima).
