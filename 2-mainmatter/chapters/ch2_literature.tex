...
Among the reviewed literature, the closest approaches are Wen~et~al.~\cite{verhulst_pso}, who employed a Verhulst-law-based ODE regularization, and Wang~et~al.~\cite{wang_pinn}, who applied PINNs with the full electrochemical SPM model. Wen~et~al.\ did not compare fixed against learnable physics parameters nor include Double-Exponential degradation priors, and their method was not designed for edge deployment. Wang~et~al.\ demonstrated high-fidelity PDE-PINNs, but their computational cost and memory footprint make them unsuitable for on-device BMS inference. In contrast, the present work combines lightweight GRU encoders with simplified ODE-based physics priors, integrates ICA/DVA features, and empirically evaluates both fixed and uncertainty-based loss weighting while explicitly constraining model complexity for MCU deployment. This positions the proposed approach as a simplified and computationally practical alternative to existing PINN and hybrid methods.
