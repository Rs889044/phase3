...
\textbf{The Double-Exponential Model:} To address the rigidity of the logistic model, a Double-Exponential form ($y = a \cdot e^{bx} + c \cdot e^{dx}$) is widely used; Yuan et al. \cite{ma_dual_exp} recently built an RUL predictor around it by combining the dual-exponential degradation law with an extended Kalman filter.
...
\textbf{LSTM vs. GRU:} Recent comparative work continues to find recurrent architectures competitive baselines for time-series forecasting \cite{zhang_rnn}, and LSTMs in particular remain a standard reference in battery prognostics. Jafari et al. \cite{jafari_cnn_gru} adopt Gated Recurrent Units (GRUs) in place of LSTMs, noting comparable accuracy at lower computational cost (two gates vs. three).
...
Much of the recent accuracy literature is built on Transformer models, which use self-attention to capture global dependencies in time-series data.
...
The seminal work in PINNs involves using neural networks to solve the governing PDEs of a system. In the battery domain, Xue et al. \cite{wang_pinn} applied PINNs to the Single Particle Model (SPM).
