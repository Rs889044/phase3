...
The gold standard for physical fidelity is the electrochemical model, often described by the \textbf{Doyle-Fuller-Newman (DFN)} framework or the \textbf{Single Particle Model (SPM)}. These models use coupled Partial Differential Equations (PDEs) to describe lithium-ion concentration gradients, potential distributions, and electrolyte dynamics. Xue et al. \cite{wang_pinn} demonstrated that such formulations can achieve high accuracy by resolving the cell's internal electrochemical states, including electrolyte dynamics. They provide a transparent view of internal degradation, allowing engineers to pinpoint whether capacity fade is due to anode degradation or electrolyte depletion.
...
\textbf{The Double-Exponential Model:} To address the rigidity of the logistic model, a Double-Exponential form ($y = a \cdot e^{bx} + c \cdot e^{dx}$) is widely used: He et al. \cite{he_calce} established it on the CALCE prismatic cells studied in this thesis, and Yuan et al. \cite{ma_dual_exp} recently built an RUL predictor around it by combining the dual-exponential degradation law with an extended Kalman filter.
...
\textbf{Note on Classification:} Each referenced work in the bibliography has been categorized by approach (PDE-PINN, empirical ODE, particle filter, or data-driven) in the Appendix. This explicit mapping clarifies that most recent PINN work in batteries attempts to embed high-fidelity PDEs; ODE-regularized hybrids remain a small subset, and within it, to the best of our knowledge, no prior work compares fixed against \emph{learnable} ODE parameters under a leave-one-cell-out protocol, or validates uncertainty weighting against a tuned fixed weight --- the two comparisons this thesis contributes.

\section{Data-Driven Approaches}

The availability of high-quality open-source datasets \cite{severson_data} has fueled the dominance of data-driven methods, which treat the battery as a ``black box'' and learn the mapping from inputs to SOH.
...
\textbf{LSTM vs. GRU:} Recent comparative work continues to find recurrent architectures competitive baselines for time-series forecasting \cite{zhang_rnn}, and LSTMs \cite{hochreiter_lstm} in particular remain a standard reference in battery prognostics. Jafari et al. \cite{jafari_cnn_gru} adopt Gated Recurrent Units (GRUs) \cite{cho_gru} in place of LSTMs, noting comparable accuracy at lower computational cost (two gates vs. three).
...
Much of the recent accuracy literature is built on Transformer models \cite{vaswani_attention}, which use self-attention to capture global dependencies in time-series data.
...
\textbf{Limitations:} Despite their accuracy, Transformers have a quadratic memory complexity ($O(L^2)$) with respect to sequence length \cite{vaswani_attention}, making them difficult to deploy on resource-constrained hardware compared to the linear complexity ($O(L)$) of RNNs. Equally important, their reported accuracy is obtained on large training corpora: self-attention models require substantial data, and in low-data regimes their excess capacity leads to overfitting rather than higher accuracy. This matters for the BMS setting, where run-to-failure data for a given cell design is scarce; this thesis tests the point empirically by benchmarking two Transformer scales at realistic data volumes (Chapter~\ref{ch:results}). Furthermore, purely data-driven models like SGEformer are prone to overfitting to dataset artifacts, such as sensor noise masquerading as capacity regeneration, leading to physically implausible predictions (e.g., predicting infinite battery life in certain local minima).

\textbf{Comparative Note:} For practical on-device feasibility, GRU-based hybrid solutions offer a favorable accuracy--cost trade-off; Transformers yield strong accuracy but at a compute and memory cost that is difficult to justify for on-device deployment.

\section{Hybrid Physics-Informed Frameworks}

Physics-Informed Machine Learning (PIML) merges the interpretability of physics with the flexibility of deep learning, and is the family this thesis belongs to.

\subsection{PDE-Based PINNs (The ``Heavy'' Approach)}

The seminal work of Raissi et al. \cite{raissi_pinn} established the PINN framework: a neural network is trained to solve the governing PDEs of a system by penalizing the equation residual in its loss. In the battery domain, Xue et al. \cite{wang_pinn} applied PINNs to the Single Particle Model (SPM).
...
A more pragmatic branch of PIML utilizes simpler Ordinary Differential Equations (ODEs) as regularizers. Wen et al. \cite{pde_pinn} pioneered a framework fusing a neural network with the Verhulst ODE for battery prognostics. They employed an uncertainty-based weighting method to balance the data loss and physics loss, demonstrating improved generalization. Earlier, Xian et al. \cite{verhulst_pso} had established the Verhulst law itself as a viable degradation model within a particle-filter framework.
...
\textbf{Novelty Statement:} While prior studies have embedded ODE priors into neural networks, this research addresses a distinct gap in the literature by integrating four specific components into a unified framework: (a) a Gated Recurrent Unit (GRU) backbone selected for edge-compatibility; (b) the use of Double-Exponential priors alongside Verhulst models to capture accelerated "knee-point" degradation, in both a \emph{fixed} closed-form variant and a \emph{learnable} ODE-residual variant whose parameters train jointly with the network; (c) an empirical evaluation of homoscedastic uncertainty weighting against a tuned fixed loss weight in this setting --- a comparison largely absent from the battery literature; and (d) physics-informed ICA/DVA feature engineering. This specific architectural combination, together with its measured edge-deployment profile, constitutes the primary novelty of this thesis, offering a practical solution that bridges the gap between computationally expensive physical models and purely data-driven approaches.
\paragraph{Comparison with Closest Prior Works.}
Among the reviewed literature, the closest approaches are Wen~et~al.~\cite{pde_pinn}, who employed a Verhulst-law-based ODE regularization with uncertainty weighting, and Xue~et~al.~\cite{wang_pinn}, who applied PINNs with the full electrochemical SPM model. Wen~et~al.\ did not compare fixed against learnable physics parameters nor include Double-Exponential degradation priors, and their method was not designed for edge deployment. Xue~et~al.\ demonstrated high-fidelity PDE-PINNs, but their computational cost and memory footprint make them unsuitable for on-device BMS inference. In contrast, the present work combines lightweight GRU encoders with simplified ODE-based physics priors, integrates ICA/DVA features, and empirically evaluates both fixed and uncertainty-based loss weighting while explicitly constraining model complexity for MCU deployment. This positions the proposed approach as a simplified and computationally practical alternative to existing PINN and hybrid methods.
