...
To address the query about data set sufficiency, the project uses two of the most widely cited datasets in the battery domain \cite{nasa_datasets, he_calce}. Their specific characteristics, artifacts and strategic roles in this research are detailed in Table \ref{tab:datasets} below.
...
\midrule
\textbf{Shared physics-informed features} & \multicolumn{2}{>{\centering\arraybackslash}p{\dimexpr0.78\textwidth-2\tabcolsep\relax}@{}}{\texttt{ica\_peak\_voltage\_V}, \texttt{ica\_peak\_magnitude}, \texttt{ica\_area} (from $dQ/dV$)} \\
\midrule
\textbf{Electrical / state features} & \multicolumn{2}{>{\centering\arraybackslash}p{\dimexpr0.78\textwidth-2\tabcolsep\relax}@{}}{\texttt{discharge\_time\_s}, \texttt{v\_min}, \texttt{v\_mean}, \texttt{v\_var}, \texttt{ir\_proxy\_ohm}} \\
\midrule
Dataset-specific diagnostics (not model inputs) & \texttt{temp\_rise\_C} (per-cycle temperature rise) & \texttt{internal\_resistance\_Ohm} (measured; the source of \texttt{ir\_proxy\_ohm}) \\
\midrule
Record count & tens of thousands of raw points per cell & 260{,}931 raw Channel records \\
Tensorization & \multicolumn{2}{>{\centering\arraybackslash}p{\dimexpr0.78\textwidth-2\tabcolsep\relax}@{}}{MinMax scaling (train cells only) + sliding window; sequence length 8--15 (set by HPO)} \\
\bottomrule
\end{tabularx}
\end{table}
...
representation, so the model sees a consistent input space across datasets; the
dataset-specific columns in the table are carried for diagnostics and are not
model inputs. One column requires a caution: \texttt{ir\_proxy\_ohm}
holds a directly measured internal resistance on CALCE but a $|\Delta V/\Delta I|$
onset proxy on NASA --- two related but physically different quantities under one
column name, which contributes to the feature shift in the cross-dataset transfer
(Chapter~\ref{ch:results}, Section~\ref{sec:cross_dataset}).
Figure~\ref{fig:feature_corr} shows that these features are strongly correlated
with SOH on every cell.

\noindent
\textbf{Feature--label proximity.} The strength of these correlations must be read
with care, because two features are close relatives of the label itself. Under a
constant-current discharge, \texttt{discharge\_time\_s} is approximately capacity
divided by current --- very nearly a rescaled copy of the same-cycle capacity from
which the SOH label is computed --- and \texttt{ica\_area} approximates the charge
throughput of the same cycle. Two consequences follow and are kept in view when
reading Chapter~\ref{ch:results}. First, the pointwise SOH task is made
substantially easier by these near-proxy inputs, which is one reason every
competent model lands within a narrow error band and the informative differences
between models appear in physical consistency, prognostic calibration, and
robustness rather than in raw RMSE. Second, a production BMS rarely observes
complete constant-current discharges (real usage is partial and irregular), so
the per-cycle availability of these features is itself an assumption of the
laboratory datasets. An ablation with same-cycle capacity proxies excluded
(for example, features lagged by one cycle, or charge-side ICA features only) is
identified as worthwhile future work.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        ICA feature extraction and for visualization.
    ...
    $v_\text{mean}$, intra-cycle voltage variance, IR proxy). A per-cycle
    temperature-rise summary and the ICA peak prominence are also computed, but
    as diagnostics: the models receive only the shared eight features plus the
    cycle coordinate. A few cycles whose preceding charge log is
    corrupted (B0005/06/07 cyc.~30; B0018 cyc.~45,\,55) are flagged as
    ICA-unusable and imputed from their neighbours rather than dropped. (The
    charge--discharge pairing itself is made by operation order in the raw
    \texttt{.mat} sequence --- the last charge preceding each discharge ---
    with an assert; a positional pairing would silently shift the ICA features
    by one to two cycles past the corrupted extra charge operations.)
...
\begin{itemize}
    ...
    Savitzky--Golay smoothing (window~11, polyorder~3); on this dataset the
    measured internal-resistance column supplies \texttt{ir\_proxy\_ohm} directly.
...
\noindent
\textbf{Additional data-handling notes.} (i) Missing feature values are imputed
within each cell only: forward-fill, then backward-fill for leading gaps, then the
cell median. The backward-fill step assigns a later cycle's value to the first few
cycles of a record --- a small non-causal convenience that is acceptable offline
and is disclosed here because the deployment framing is online. (ii) On CALCE the
charge current is reconstructed by numerically differentiating the cumulative
charge-capacity column, so the ICA feature applies two numerical derivatives to
the same signal; Savitzky--Golay smoothing precedes each differentiation, but the
NASA and CALCE ICA features are consequently not computed from identical raw
signals.
...
The predictive performance is quantified using standard regression metrics:
\begin{align}
...
\end{align}
RUL accuracy is assessed with the prognostic (PHM) metrics of Saxena et
al.~\cite{saxena_metrics}, applied in Chapter~\ref{ch:results}
(Section~\ref{sec:phm}) --- the $\alpha$-$\lambda$ accuracy and Cumulative
Relative Accuracy --- rather than a single pointwise RUL error, since a lone
terminal error hides how the horizon estimate evolves over the cell's life.
...
\begin{itemize}
    ...
    \item \textbf{Data Efficiency Study:} behaviour in data-sparse regimes is probed by two training-set-reduction studies: a data-volume ablation on the hybrid (25\%--100\% of the training windows) and a data-scarcity stress test against both Transformer scales (10\%--100\%); results in Chapter~\ref{ch:results}, Sections~\ref{sec:overfitting} and~\ref{sec:ablations}.
\end{itemize}
...
% Single-space the cell text (the table inherits the document's 1.5 line
% spacing otherwise, making it taller than the text block and pushing its
% bottom rule and caption into the footer).
...
\textbf{Data-Driven (Classical)} & Support Vector Regression (RBF) & \textbf{Non-Recurrent Control:} A strong shallow regressor on the same windowed features. Isolates the value of \emph{temporal} sequence modeling: if the recurrent networks do not beat it, sequence modeling adds no value on these features. \\
...
\textbf{Physics-Informed (Alternative)} & Neural-ODE & \textbf{PIML Control:} A continuous-time physics-informed alternative (a last-observation latent-ODE head). Ensures the proposed hybrid is compared against another physics-motivated architecture, not only black boxes. \\
...
\medskip
\noindent \textbf{Tuning budget.} The baselines did not all receive an identical
hyperparameter search, and the differences are stated here so the comparison can
be judged fairly. The GRU, the light Transformer and both hybrid routes carry
configurations from the recorded Optuna study on the development fold
(Chapter~\ref{ch:methodology}); the LSTM and the Neural-ODE use hand-set values
within the same ranges (hidden size 16--32, learning rate
$10^{-3}$--$5\times10^{-3}$); the SVR uses standard fixed settings (RBF kernel,
$C=10$, $\varepsilon=0.005$) without a search; and the heavy Transformer is
deliberately hand-configured with strong regularization rather than tuned, since
a proper search at 225k parameters is not feasible with four cells (its scoping
note appears in Chapter~\ref{ch:results}, Section~\ref{sec:overfitting}). The
double-exponential fit has no hyperparameters beyond its functional form.

\medskip
\noindent \textbf{Sufficiency of Data Volume:} While deep learning approaches typically require massive labeled datasets to generalize, \textbf{Physics-Informed models are hypothesized to be data-efficient}: the integration of physical equations constrains the optimization search space, guiding the model even when data is sparse. Rather than assume this, the thesis tests it directly --- via a data-volume ablation on the hybrid and a data-scarcity stress test pitting the hybrid against both Transformer scales at 10--100\% of the training cycles (Chapter~\ref{ch:results}, Sections~\ref{sec:overfitting} and~\ref{sec:ablations}). The combined cycle count of the NASA and CALCE datasets is sufficient for this hypothesis test precisely because the small-data regime is the regime of interest.
...
int8 model size (PyTorch dynamic quantization, qnnpack engine), per-inference
...
MACs measured; int8 size and latency are the clearly-labeled estimates described
...
Profiling method & PyTorch dynamic int8 quantization (qnnpack), measured; analytic latency \\
...
``lightweight'' claim is made on equal footing. After measured int8 quantization
...
\begin{itemize}
    ...
    (PyTorch, qnnpack engine), with the quantized model serialized and its size
    \emph{measured}. Linear and recurrent layers are quantized; attention
    ...
    (CMSIS-NN-style) estimate, clearly labeled as such rather than an on-board
    measurement. On-board validation with a quantized export (ONNX or
...
quantization, at 60\% of the flash budget, while the deployable hybrid uses
