...
To address the query about data set sufficiency, the project uses two of the most widely cited datasets in the battery domain. Their specific characteristics, artifacts and strategic roles in this research are detailed in Table \ref{tab:datasets} below.
...
input space across datasets. One column requires a caution: \texttt{ir\_proxy\_ohm}
...
\begin{itemize}
    ...
    temperature-rise feature. A few cycles whose preceding charge log is
...
\end{align}
RUL accuracy is assessed with the prognostic (PHM) metrics of
Chapter~\ref{ch:results} (Section~\ref{sec:phm}) --- the $\alpha$-$\lambda$
accuracy and Cumulative Relative Accuracy --- rather than a single pointwise
RUL error, since a lone terminal error hides how the horizon estimate evolves
over the cell's life.
