...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        physical consistency must be earned against real regeneration, not
        against a pre-sanitised label. Savitzky--Golay smoothing is used only
        inside the ICA feature extraction and for visualisation.
...
\]
where $\mathbb{I}$ is the indicator function. A higher PCS indicates fewer physical violations (e.g., self-healing artifacts). In implementation, consecutive-cycle increases smaller than $10^{-3}$ SOH are counted as monotone --- a numerical tolerance that absorbs floating-point noise without masking genuine violations.
...
per-inference MACs, and peak activation RAM are \emph{measured} (state-dict
serialisation and forward hooks on the trained models); the \emph{int8 flash}
figure is the standard $4\times$ post-training-quantisation estimate of the fp32
weight size; and the Cortex-M4 latency is an analytic MACs-per-clock
(CMSIS-NN-style) estimate. The full footprint table for the hybrid, GRU and both
Transformer scales is reported in Chapter~\ref{ch:results},
...
Profiling method & PyTorch fp32 profiling; int8 $=$ $4\times$ PTQ estimate; analytic latency \\
...
\begin{itemize}
    ...
    reduces weight storage by $4\times$ versus float32; the int8 flash figures are
    this $4\times$ estimate applied to the measured fp32 size, not a serialised
    quantised artifact.
