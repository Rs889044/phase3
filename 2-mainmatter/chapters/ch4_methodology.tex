...
\textbf{Implementation Notes:} The GRU hidden size (32--64) and sequence length (8--15 cycles) were selected by Optuna \cite{akiba_optuna} Bayesian (TPE) hyperparameter optimization. Parameter counts range from $\approx$4k (vanilla GRU) to $\approx$41k (the hidden-64 benchmark hybrid), with the deployed matched-size hybrid at $\approx$11k --- all well under a 1~MB footprint (see Chapter~\ref{ch:results}, Section~\ref{sec:footprint}).

\section{Adaptive Loss Balancing (Uncertainty Weighting)}
\label{sec:uncertainty_weighting}
...
To solve this, we adopt the \textbf{Homoscedastic Uncertainty Weighting} approach proposed by Kendall et al. \cite{kendall_uncertainty} and applied to batteries by Wen et al. \cite{pde_pinn}. This method interprets the multi-objective loss minimization as maximizing the Gaussian log-likelihood of the model. The composite loss function is reformulated as:
...
\textbf{Implementation note:} the weighting is implemented in log-variance form, $\sum_k e^{-s_k}\mathcal{L}_k + \tfrac{1}{2}s_k$ with learnable $s_k = \log\sigma_k^2$ --- a Kendall-\emph{style} variant of the equation above, equivalent up to a factor-two rescaling of the effective task weight, which the learnable $s_k$ absorbs during training.
...
\begin{itemize}
    ...
    with the network; the physics loss penalizes the residual between the network's
...
\textbf{homoscedastic uncertainty weighting} of Section~\ref{sec:uncertainty_weighting}. The hyperparameters
(hidden size, sequence length, learning rate, physics weight) are selected by
\textbf{Optuna} Bayesian (TPE) optimization, which is distinct from the Bayesian
...
proved the most robust, the fixed closed-form prior (route~A) was less reliable,
and uncertainty weighting brought no reliable accuracy benefit over a well-chosen
fixed weight in this setting (its one consistent effect, a physical-consistency
gain on route~B, is quantified in Chapter~\ref{ch:results}).