...
The transition from internal combustion engines (ICE) to electric propulsion is one of the major industrial shifts of the 21st century. As nations work toward the climate goals of the Paris Agreement and subsequent COP summits, the demand for high-performance, reliable energy storage systems has grown rapidly.
...
The development of accurate and computationally feasible prognostic algorithms is therefore a practical requirement for the large-scale adoption of electric mobility, not only an academic problem.
...
\begin{itemize}
    ...
    \item \textbf{Validation on Diverse Datasets:} To validate the model's robustness and generalizability using distinct datasets (short-life NASA PCoE cells with heavy capacity regeneration; the long-life CALCE prismatic cell with a different load profile and form factor), focusing on the model's ability to handle data artifacts like ``capacity regeneration'' without making physically impossible predictions.
...
\begin{enumerate}
    \item \textbf{A leak-free, reproducible benchmark of ten models} for SOH/RUL estimation on the NASA and CALCE datasets --- two non-neural floors, four purely data-driven networks (LSTM, GRU, and two Transformer scales), a Neural-ODE, a matched-capacity no-physics control, and the two proposed hybrid variants --- all evaluated under leave-one-cell-out with identical inputs, seeds, and metrics (Chapters~\ref{ch:datasets_implementation} and~\ref{ch:results}).
    \item \textbf{A head-to-head comparison of fixed against learnable physics} within the same framework: the integrated closed-form prior (route~A) versus a learnable ODE residual (route~B). The learnable variant is better than or equal to the fixed prior on every held-out cell, and a matched-capacity control attributes its accuracy gain to the physics loss rather than to model capacity (Chapter~\ref{ch:results}).
    \item \textbf{An empirical test of homoscedastic uncertainty weighting} against a tuned fixed loss weight for the data/physics balance, on the same full evaluation protocol as the benchmark, with the outcome reported as measured (Chapter~\ref{ch:results}).
    \item \textbf{A measured Cortex-M4 deployment profile} of the proposed model against both Transformer scales --- int8 quantization, flash, activation RAM, MAC counts, and an analytic latency estimate --- with the accuracy cost of the deployment size stated explicitly (Chapter~\ref{ch:results}).
    \item \textbf{A corrected, documented data pipeline} for the NASA and CALCE datasets, including the charge--discharge pairing fix, the CALCE timeline assembly, and label definitions enforced by automated invariant checks (Chapter~\ref{ch:datasets_implementation}).
\end{enumerate}
...
\textbf{Chapter 2} presents a critical review of the literature, categorizing existing methodologies into model-based, data-driven, and hybrid paradigms, and identifies specific research gaps.
...
\textbf{Chapter 6} presents the measured experimental results: the model benchmark, the central finding on learnable vs.\ fixed physics, the quantified gap to the high-capacity Transformer (including the data-scarcity and overfitting stress tests), the prognostic (PHM) assessment, the ablation studies, and the deployment footprint.
