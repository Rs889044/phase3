...
\begin{itemize}
    ...
    step and integrating a learned latent field is too weak for this task; the
    result is reported as measured. The curve fit also transfers
    ...
    \item \textbf{Additional attention capacity does not improve accuracy.} The
    ...
    \item \textbf{The LSTM average masks a per-cell weakness.} Its wide interval
    ...
    to $0.2617$} out of distribution, a negative result analyzed in
...
coordinate to every model is a design choice, and its cost is measurable. In the
pre-correction run the baselines did not receive any time input (the leaked
coordinate went only to the hybrids), so those numbers serve as a no-time-input
reference under otherwise identical folds and seeds: the coordinate helped the
GRU ($0.0417 \rightarrow 0.0359$) and the light Transformer
($0.0395 \rightarrow 0.0345$), but hurt the LSTM
($0.0331 \rightarrow 0.0359$) and the Neural-ODE
($0.0410 \rightarrow 0.0505$) --- consistent with the design ablation, where the
raw coordinate without a physics term does not reliably help
(Section~\ref{sec:ablations}). The benchmark conclusion does not depend on this
choice: route~B ($0.0262$) is ahead of the strongest baseline in either
configuration, including the LSTM's no-time-input $0.0331$.
...
\begin{figure}[H]
    ...
    the training population suggests; over-estimation is the more serious error
...
regularization (dropout 0.3, weight decay, a longer schedule), which indicates
...
(10--100\% of the training cycles) and a train--test generalization-gap
...
degrades to $0.2617$, the weakest cross-dataset result among the learned
models.} The
...
LOCO results are the primary evidence of this thesis.
...
inherently uncertain, and models without a usable degradation prior are least
reliable in that region. Figure~\ref{fig:alpha_lambda} shows the $\alpha$-$\lambda$ RUL
...
\begin{enumerate}
    ...
    protocol the accuracy gain belongs to the physics term. Because this
    decomposition supports a central claim of the thesis, confirming it on the
    full four-fold, three-seed protocol is noted as a worthwhile follow-up.
    ...
    used for the main benchmark models.
...
dynamic quantization with the qnnpack engine; linear and recurrent layers are
quantized, attention in-projections remain fp32, which is why the Transformers
...
accuracy ($0.0262 \rightarrow 0.0328$), but the deployed model remains
nominally ahead of every baseline in the benchmark --- clearly more accurate
than the heavy Transformer ($0.0418$) with $20\times$ fewer parameters,
$30\times$ less flash, $6\times$ less RAM and $11\times$ lower estimated
latency, and nominally ahead of the light Transformer ($0.0345$; the two
confidence intervals overlap) at $3.5\times$ less flash with better physical
consistency ($0.893$ vs.\ $0.783$). Figure~\ref{fig:frontier} summarizes the
...
\begin{figure}[H]
    ...
    hybrid lies in the lower-left region of the accuracy--memory plane: more
    accurate than both
...
\begin{itemize}
    ...
    quantization, qnnpack engine); the M4 latency is analytic and conservative
...
\begin{itemize}
    ...
    formal significance.
    \item \textbf{Negative results.} The homoscedastic
...
physics regularizer is the best-balanced choice for deployable battery
prognostics in-distribution, and a heavyweight attention model is neither
small enough for the edge nor more accurate at realistic data volumes. The
framework's remaining weakness, long-horizon extrapolation of the cycle
coordinate across datasets, is identified and explained, and has clear
mitigations.
